...
```{r, fig.height = fig_height_h_barchart(n_y=1, n_cats_y=5, max_chars_cats_y=18, multiplier_per_plot=.3)}
...
## I hvilken grad stemmer disse utsagnene?{#sec-I-hvilken-grad-stemmer-disse-utsagnene--90}
...
## Lederutdanningen har gitt meg kompetanse til å{#sec-Lederutdanningen-har-gitt-meg-kompetanse-til-a-94}


::: {#fig-utbytte-q10-5-fct-cat-plot-html}

```{r, fig.height = fig_height_h_barchart(n_y=3, n_cats_y=6, max_chars_cats_y=14)}
utbytte_q10_5_fct_cat_plot_html <-
	data_4_Utbytte |>
		makeme(dep = c(Q10.5_1,Q10.5_2,Q10.5_3),
		type = 'cat_plot_html')
nrange <- stringi::stri_c('N = ', n_range2(utbytte_q10_5_fct_cat_plot_html[[1]]))
link <- make_link(data = utbytte_q10_5_fct_cat_plot_html[[1]]$data)
link_plot <- make_link(data = utbytte_q10_5_fct_cat_plot_html,
		file_suffix = '.png', link_prefix='[PNG](',
		save_fn = ggsaver)
x <- I(paste0(c(nrange, link, link_plot), collapse=', '))
girafe(utbytte_q10_5_fct_cat_plot_html)
```
...
## På en skala fra 1 (helt uenig) til 6 (helt enig), vennligst gi din oppfatning av læringsutbyttet av lederutdanningen{#sec-Pa-en-skala-fra-1-helt-uenig-til-6-helt-enig-vennligst-gi-din-oppfatning-av-laringsutbyttet-av-lederutdanningen-11}
